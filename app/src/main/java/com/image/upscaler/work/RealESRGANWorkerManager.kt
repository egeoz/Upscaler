...
class RealESRGANWorkerManager(
    ...
    init {
        ...
        workProgressFlow = MutableStateFlow<InputAndProgress?>(null).apply {
            currentWorkerLiveData.observeForever { currentWorker ->
                tryEmit(workInfosLiveData.value?.let { infos ->
                    processWorkInfo(
                        infos,
                        currentWorker
                    )
    ...
    fun beginWork(input: RealESRGANWorker.InputData) {
        if (queueWorkJob?.isCompleted != false) {
            queueWorkJob = processLifecycle.lifecycleScope.launch(Dispatchers.IO) {
                 ...
                 */
                val running =
                    workManager.getWorkInfosForUniqueWork(UNIQUE_WORK_ID).await().firstOrNull()
                        ?.let {
                            RealESRGANWorker.Progress.fromWorkInfo(it) is RealESRGANWorker.Progress.Running
                        } == true
                if (!running) {
                    val request =
                        OneTimeWorkRequestBuilder<RealESRGANWorker>().setInputData(input.toWorkData())
                            .build()
    ...
    companion object {
        ...
        private fun processWorkInfo(
            workInfos: List<WorkInfo>,
            currentWorker: Pair<UUID, RealESRGANWorker.InputData>?
        ): InputAndProgress? {
