...
class RealESRGANWorker(
    ...
    private val progressNotificationBuilder =
        NotificationCompat.Builder(applicationContext, PROGRESS_NOTIFICATION_CHANNEL_ID)

    private val inputImageTempFile =
        params.inputData.getString(INPUT_IMAGE_TEMP_FILE_NAME_PARAM)?.let {
            RealESRGANWorkerManager.getTempFile(context, it)
        }
    private val inputImageName =
        params.inputData.getString(INPUT_IMAGE_NAME_PARAM) ?: throw IllegalArgumentException(
    ...
    private fun setupProgressNotificationBuilder() = with(applicationContext) {
        progressNotificationBuilder.apply {
            setTitleAndTicker(
                getString(
                    R.string.upscaling_worker_notification_title,
                    inputImageName
                )
            )
    ...
    private fun getInputBitmap(): Bitmap? = inputImageTempFile?.inputStream()?.use {
        ...
        BitmapFactory.decodeStream(it, null, options)
            ?.transformWithExif(ExifInterface(inputImageTempFile))
    }
    ...
    private fun buildProgressNotification(progress: JNIProgressTracker.Progress?): Notification =
        progressNotificationBuilder.apply {
            val value = progress?.value ?: JNIProgressTracker.INDETERMINATE_PROGRESS
            val estimatedMillisLeft =
                progress?.estimatedMillisLeft ?: JNIProgressTracker.INDETERMINATE_TIME
    ...
    private fun buildResultNotification(dataResult: DataResult<Uri, Int>?) =
        with(applicationContext) {
            ...
            if (dataResult is DataResult.Success) {
                notificationBuilder.apply {
                    setTitleAndTicker(
                        getString(
                            R.string.upscaling_worker_success_notification_title,
                            inputImageName
                        )
                    )
                    setSmallIcon(R.drawable.outline_photo_size_select_large_24)
                ...
                notificationBuilder.apply {
                    if (InterpreterError.fromNativeErrorEnum((dataResult as? DataResult.Error)?.error) == InterpreterError.CREATE_SESSION) {
                        ...
                        setTitleAndTicker(
                            getString(
                                R.string.upscaling_worker_error_notification_title,
                                inputImageName
                            )
                        )
                    }
